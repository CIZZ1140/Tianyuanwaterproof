...
def process_and_upload(url, r2_key):
    print(f"Downloading image from Alibaba CDN: {url}...")
    response = requests.get(url)
    if response.status_code != 200:
        print(f"Failed to download. Status: {response.status_code}")
        return

    # 1. Open and Optimize via PIL
    img = Image.open(BytesIO(response.content))
    
    # 2. Resize to 16:9 800px width (Standard for category cards)
    width = 800
    ...
    # 4. Upload to R2
    print(f"Uploading optimized WebP to R2: {r2_key}...")
    s3.upload_fileobj(
        webp_io,
        BUCKET_NAME,
        r2_key,
...
if __name__ == "__main__":
    target_url = "https://sc04.alicdn.com/kf/Hf077bff4e0d44a21849714651f492df4N.jpg"
    target_key = "cat-waterproof-coatings-v2.webp"
