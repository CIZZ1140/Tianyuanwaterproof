---
title: "EPDM Rubber Waterproofing Membrane"
category: "polymer-membranes"
featured: true
order: 85
product_category: "Polymer Membranes"
image: "https://img.tyuanwaterproof.com/products/epdm-rubber-waterproofing-membrane.webp"
main_image: "https://img.tyuanwaterproof.com/products/epdm-rubber-waterproofing-membrane.webp"

hero_description: "Factory-supplied premium EPDM rubber waterproofing membrane for roofing, basements, tunnels, and high-demand engineering projects. Exceptional durability with a service life of up to 50 years."

hero_tags:
  - "EPDM Rubber"
  - "GB/T 18173.1-2012"
  - "1.0mm–2.0mm"
  - "20m/Roll"
  - "Aging Resistance"
  - "OEM Available"

gallery:
  - image: "https://img.tyuanwaterproof.com/products/epdm-rubber-waterproofing-membrane.webp"
    title: "EPDM Membrane Roll"
  - image: "https://img.tyuanwaterproof.com/home/factory.webp"
    title: "Production Line"

spec_table:
  - label: "Product Type"
    value: "EPDM Rubber Waterproofing Membrane"
  - label: "Standard"
    value: "GB/T 18173.1-2012"
  - label: "Material"
    value: "Ethylene-propylene diene monomer rubber waterproofing material"
  - label: "Thickness"
    value: "1.0mm, 1.2mm, 1.5mm, 2.0mm"
  - label: "Width"
    value: "1m, 1.2m (Customizable up to 4m)"
  - label: "Length"
    value: "20m/roll (Customizable)"
  - label: "Color"
    value: "Standard Black"
  - label: "Type"
    value: "H: Homogeneous; L: Fabric-backed"
  - label: "Application"
    value: "Roofing, basement, tunnel, bridge, subway, cisterns, dams"
  - label: "OEM Support"
    value: "Thickness, width, length, backing, packaging, and private label"

quick_facts:
  - title: "50-Year Service Life"
    text: "Exceptional anti-aging performance, weather resistance, and ozone resistance for long-term protection."
  - title: "High Elongation"
    text: "Superior elongation (≥450%) and tensile strength to accommodate significant structural movement."
  - title: "Extreme Temp Range"
    text: "Maintains flexibility at -40°C and stability under damp-heat conditions, suitable for all climates."
  - title: "Eco-Friendly Rubber"
    text: "Non-toxic, environmentally friendly, and safe for reservoirs, dams, and potable water projects."

applications:
  - title: "Civil & Industrial Roofing"
    text: "The preferred solution for high-durability roof waterproofing in residential and commercial buildings."
  - title: "Underground Engineering"
    text: "Reliable barrier for basements, subways, and underground municipal structures."
  - title: "Hydraulic Engineering"
    text: "Suitable for cisterns, dams, pools, and reservoirs requiring high chemical resistance."
  - title: "Bridge & Road Works"
    text: "Used for bridge deck waterproofing and motorways to prevent water ingress and corrosion."

features:
  - title: "Unmatched Aging Resistance"
    text: "EPDM molecular structure provides the best aging resistance among all polymer waterproofing materials."
  - title: "Chemical & Heat Stability"
    text: "Resistant to oil, solvents, and chemical corrosion with excellent performance under heat treatment."
  - title: "Reliable Joint Bonding"
    text: "Solid lap joint treatment using professional adhesives or seam tapes ensures a monolithic seal."
  - title: "Perforation Resistance"
    text: "High mechanical strength and puncture resistance protect the membrane during and after construction."

technical_data:
  - label: "Applicable Standard"
    unit: "—"
    standard: "GB/T 18173.1-2012"
    requirement: "Standard Conformity"
    value: "Pass"
  - label: "Tensile Strength"
    unit: "MPa"
    standard: "GB/T 18173.1"
    requirement: "Vulcanized ≥ 7.5"
    value: "9.0+"
  - label: "Elongation at Break"
    unit: "%"
    standard: "GB/T 18173.1"
    requirement: "Vulcanized ≥ 450"
    value: "500% - 600%"
  - label: "Tear Strength"
    unit: "kN/m"
    standard: "GB/T 18173.1"
    requirement: "Vulcanized ≥ 25"
    value: "30+"
  - label: "Low Temp Bending"
    unit: "°C"
    standard: "GB/T 18173.1"
    requirement: "-40°C"
    value: "No crack"
  - label: "Impervious To Water"
    unit: "MPa"
    standard: "GB/T 18173.1"
    requirement: "0.3MPa, 30min"
    value: "Watertight"
  - label: "Joint Peel Strength"
    unit: "N/mm"
    standard: "GB/T 18173.1"
    requirement: "H type ≥ 4.0"
    value: "Passed"
  - label: "Ozone Aging"
    unit: "—"
    standard: "40°C, 500pphm"
    requirement: "No cracking"
    value: "Passed"

application_guide:
  - title: "Substrate Prep"
    text: "Ensure the base course is flat, solid, dry, and free from cracks, oil stains, or dust."
  - title: "Primer Treatment"
    text: "Apply compatible primer evenly on the base and allow it to dry properly before paving."
  - title: "Pre-Lay Membrane"
    text: "Unroll the EPDM membrane along layout lines and allow it to relax for 30 minutes."
  - title: "Adhesive Bonding"
    text: "Apply adhesive on both the substrate and membrane underside, avoiding overlap areas."
  - title: "Pave & Roll"
    text: "Lay the membrane smoothly, exhausting air with a roller to ensure full bonding."
  - title: "Joint Sealing"
    text: "Treat long-side (≥100mm) and short-side (≥150mm) laps with adhesive or professional seam tape."

packaging:
  - title: "Roll Packaging"
    text: "Supplied in heavy-duty rolls wrapped with protective end caps and woven bags."
  - title: "Safe Storage"
    text: "Stack separately below 50°C in a ventilated area, away from direct sun and rain."
  - title: "OEM Labeling"
    text: "Custom roll labels with your logo, type, thickness, and batch number available."

loading_data:
  - label: "20FT Container"
    value: "Approx. 5,000–8,000 m² (Depending on thickness)"
  - label: "Storage Period"
    value: "One year under standard conditions"

downloads:
  - title: "EPDM Technical Data Sheet"
    description: "Detailed GB/T 18173.1 specifications and performance benchmarks."
    type: "PDF"
    href: ""
  - title: "Installation Guide"
    description: "Full construction guidance for adhesive bonding and detail treatment."
    type: "PDF"
    href: ""

related_products:
  - title: "TPO Waterproofing Membrane"
    image: "https://img.tyuanwaterproof.com/products/tpo-waterproof-membrane.webp"
    href: "/products/tpo-waterproofing-membrane-manufacturer/"
  - title: "PVC Waterproofing Membrane"
    image: "https://img.tyuanwaterproof.com/products/pvc-waterproof-membrane.webp"
    href: "/products/pvc-waterproofing-membrane-manufacturer/"
  - title: "Pre-applied HDPE Membrane"
    image: "https://img.tyuanwaterproof.com/products/hdpe-waterproof-membrane.webp"
    href: "/products/pre-applied-hdpe-membrane-supplier/"
  - title: "SBS Modified Bitumen Membrane"
    image: "https://img.tyuanwaterproof.com/products/sbs-waterproof-membrane.webp"
    href: "/products/sbs-bitumen-waterproofing-membrane-manufacturer/"

faqs:
  - question: "Why is EPDM considered the longest-lasting membrane?"
    answer: "EPDM molecular structure provides unmatched resistance to UV, ozone, and thermal aging, with many systems lasting over 50 years."
  - question: "Can EPDM be used for fish ponds or reservoirs?"
    answer: "Yes, our high-quality EPDM is non-toxic and environmentally friendly, making it safe for aquaculture and cisterns."
  - question: "What is the installation method for EPDM?"
    answer: "It is generally installed using a full adhesive bonding system or mechanical fastening, with joints sealed by seam tapes or adhesives."
---

# High-Durability EPDM Rubber Waterproofing

EPDM rubber waterproofing membrane is a premier synthetic polymer sheet made from ethylene-propylene diene monomer rubber. Renowned for its extreme longevity and anti-aging performance, it is the gold standard for high-demand civil and industrial waterproofing projects.

As a specialist manufacturer, Tianyuan supplies both homogeneous (H) and fabric-backed (L) EPDM sheets. With high tensile strength and a temperature range from -40°C to 160°C, our EPDM membrane provides a reliable, eco-friendly barrier for everything from luxury roofing to complex hydraulic engineering like dams and subways.
